...
func initDB(dbPath string) (*sql.DB, error) {
	...
	_, err = db.Exec(`INSERT INTO progress(time_us) SELECT 0 WHERE NOT EXISTS(SELECT * FROM progress)`)
	if err != nil {
		return db, err
	}
...
func getCursor(db *sql.DB) (int64, error) {
	var count int64
	err := db.QueryRow(`SELECT time_us FROM progress`).Scan(&count)
	if err != nil {
		return 0, err
	}
...
func setCursor(db *sql.DB, time_us int64) (error) {
	_, err := db.Exec(`UPDATE progress SET time_us = ?`, time_us)
	return err
}
